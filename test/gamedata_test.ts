...
Deno.test("fetchActiveGames returns active games from the database", async () => {
  ...
  for (const gameId of [id1, id2, id3]) {
    ...
    const gameData: GameStorageData<TestConfig, TestState, TestPlayer> = {
      config: undefined,
...
Deno.test("getPlayerState returns correct player state", async () => {
  ...
  for (const token in sessionTokens) {
    const gameData = await db.getGameStorageData<TestConfig, TestState, TestPlayer>(
      gameId,
    );
...
Deno.test("getPlayerState handles completed games", async () => {
  ...
  const initialGameData: GameStorageData<TestConfig, TestState, TestPlayer> = {
    config: undefined,
    gameState: { value: 1, moveHistory: ["Player 0: increment"] },
    sessionTokens,
    players,
...
Deno.test("getObserverState handles completed games", async () => {
  ...
  const updatedGameData = await db.getGameStorageData<TestConfig, TestState, TestPlayer>(
    gameId,
  );
...
Deno.test("handleMove rejects invalid moves", async () => {
  ...
  const initialGameData = await db.getGameStorageData<TestConfig, TestState, TestPlayer>(
    gameId,
  );
...
Deno.test("handleMove doesn't update completed games", async () => {
  ...
  // Store initial state
  const completedGameData = await db.getGameStorageData<TestConfig, TestState, TestPlayer>(
    gameId,
  );
