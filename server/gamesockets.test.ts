...
const getObserverState = (
  state: number,
  _o: ObserverStateObject<undefined>,
) => state;
...
Deno.test("only sends updates when player state changes", async () => {
  ...
  await gameSocketStore.initializePlayer(socket, gameId, 1, getPlayerState);
...
Deno.test("only sends updates when observer state changes", async () => {
  const kv = await Deno.openKv(":memory:");
  const db = new DB(kv);
